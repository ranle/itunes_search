...
module ItunesSearch
  class AppParser
    ...
    def parse
      ...
      if @doc.css('#selectedcontent a').size > 0
        @doc.css('#selectedcontent a').each do |app_content|
    ...
    def create_app(app_content)
      ...
      app.id = app.url[app.url.index('id')+2..-1]
