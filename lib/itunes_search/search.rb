...
module ItunesSearch
  class Search
    ...
    def initialize(search_condition = DEFAULT_SEARCH_CONDITION)
      ...
      @proxies = @search_condition[:proxies] if @search_condition.has_key?(:proxies)
      @username = @search_condition[:username] if @search_condition.has_key?(:username)
      @password = @search_condition[:password] if @search_condition.has_key?(:password)
      ItunesSearch::Client.new(@proxies, @username, @password)
    ...
    def search(data)
      ...
      html = ItunesSearch.get_client.get_html(init_query_url)
      # p html.html_safe
